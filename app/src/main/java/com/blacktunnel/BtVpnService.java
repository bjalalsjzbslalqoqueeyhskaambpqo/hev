...
/* ── Escritura al túnel UDP: [2 bytes longitud big-endian][payload] ────────── */
...
static void tunnel_reset(const char *reason){
    ...
    int rfd=g_relay_fd;g_relay_fd=-1;int ufd=g_udp_fd;g_udp_fd=-1;g_tun_tcp=-1;g_tun_udp=-1;g_started=0;
    pthread_mutex_unlock(&g_state_mu);
    if(reason)push_log("E","tunnel reset: %s",reason);
    if(rfd>=0)close(rfd);if(ufd>=0)close(ufd);
    q_clear();ht_clear();
...
static int open_tunnel(const char *action){
    ...
    if(inet_pton(AF_INET6,PROXY_HOST_IPV6,&a6.sin6_addr)==1){fd=socket(AF_INET6,SOCK_STREAM,0);if(fd>=0){jni_protect(fd);int v=1;setsockopt(fd,IPPROTO_TCP,TCP_NODELAY,&v,sizeof(v));int fl=fcntl(fd,F_GETFD,0);if(fl>=0)fcntl(fd,F_SETFD,fl|FD_CLOEXEC);if(connect_nb(fd,(struct sockaddr*)&a6,sizeof(a6))!=0){close(fd);fd=-1;}}}
    if(fd<0){struct addrinfo hints={0},*res=NULL;hints.ai_family=AF_UNSPEC;hints.ai_socktype=SOCK_STREAM;char ps[8];snprintf(ps,sizeof(ps),"%d",PROXY_PORT);if(getaddrinfo(PROXY_HOST,ps,&hints,&res)==0){for(struct addrinfo *r=res;r&&fd<0;r=r->ai_next){int s=socket(r->ai_family,SOCK_STREAM,0);if(s<0)continue;jni_protect(s);int v=1;setsockopt(s,IPPROTO_TCP,TCP_NODELAY,&v,sizeof(v));int fl=fcntl(s,F_GETFD,0);if(fl>=0)fcntl(s,F_SETFD,fl|FD_CLOEXEC);if(connect_nb(s,r->ai_addr,r->ai_addrlen)==0)fd=s;else close(s);}freeaddrinfo(res);}}
    if(fd<0){push_log("E","[%s] connect failed",action);return -1;}
    int one=1,idle=20,intvl=5,cnt=4;setsockopt(fd,SOL_SOCKET,SO_KEEPALIVE,&one,sizeof(one));setsockopt(fd,IPPROTO_TCP,TCP_KEEPIDLE,&idle,sizeof(idle));setsockopt(fd,IPPROTO_TCP,TCP_KEEPINTVL,&intvl,sizeof(intvl));setsockopt(fd,IPPROTO_TCP,TCP_KEEPCNT,&cnt,sizeof(cnt));
    char req1[256],h1[2048];int r1=snprintf(req1,sizeof(req1),"GET / HTTP/1.1\r\nHost: %s\r\n\r\n",PROXY_HOST);send(fd,req1,r1,MSG_NOSIGNAL);if(recv_headers(fd,h1,sizeof(h1),HANDSHAKE_TIMEOUT_SEC)<0){close(fd);return -1;}
    char req2[1024],h2[4096];struct timespec t0,t1;int r2=snprintf(req2,sizeof(req2),"- / HTTP/1.1\r\nHost: %s\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nAction: %s\r\nX-Internal-ID: %s\r\n\r\n",TUNNEL_HOST,action,g_iid[0]?g_iid:"unknown");clock_gettime(CLOCK_MONOTONIC,&t0);send(fd,req2,r2,MSG_NOSIGNAL);int hlen=recv_headers(fd,h2,sizeof(h2),HANDSHAKE_TIMEOUT_SEC);
    if(hlen<0||http_status(h2)!=101){char body[512]={0};const char *eoh=strstr(h2,"\r\n\r\n");if(eoh){int bl=hlen-(int)(eoh+4-h2);if(bl>0&&bl<(int)sizeof(body))memcpy(body,eoh+4,bl);}if(strstr(h2,"not_registered")||strstr(body,"not_registered")||strstr(h2,"no_registrado")||strstr(body,"no_registrado"))push_log("E","usuario no registrado");else if(strstr(h2,"expired")||strstr(body,"expired")||strstr(h2,"expirado")||strstr(body,"expirado"))push_log("E","usuario expirado");else push_log("E","auth error %d",http_status(h2));close(fd);return -1;}
    clock_gettime(CLOCK_MONOTONIC,&t1);long ping_ms=(t1.tv_sec-t0.tv_sec)*1000L+(t1.tv_nsec-t0.tv_nsec)/1000000L;if(ping_ms<0)ping_ms=0;
...
static void handle_tcp_conn(int cfd,int tfd,uint32_t sid){
    ...
    while(g_running){n=recv(cfd,buf,sizeof(buf),0);if(n<0){if(errno==EINTR)continue;break;}if(n==0)break;atomic_store(&g_last_traffic,(long)time(NULL));if(tun_tcp_send(T_DATA,sid,buf,(uint16_t)n)<0){tunnel_reset("T_DATA failed");break;}}
    tun_tcp_send(T_CLOSE,sid,NULL,0);ht_del(sid);
}
...
static void *tcp_tunnel_reader(void *arg){
    ...
    while(g_running&&atomic_load(&g_epoch)==epoch){
        ...
        uint8_t ft=hdr[0];uint32_t sid=((uint32_t)hdr[1]<<24)|((uint32_t)hdr[2]<<16)|((uint32_t)hdr[3]<<8)|(uint32_t)hdr[4];uint16_t len=((uint16_t)hdr[5]<<8)|hdr[6];
...
static void *udp_server_reader(void *arg){
    ...
    while(g_running&&atomic_load(&g_epoch)==epoch){
        ...
        /* Reenviar a HEV como datagrama completo */
...
static void *keepalive(void *arg){
    ...
    while(g_running&&atomic_load(&g_epoch)==epoch){
        ...
        struct timespec tp0,tp1;clock_gettime(CLOCK_MONOTONIC,&tp0);long prev=atomic_load(&g_last_pong);
        ...
        clock_gettime(CLOCK_MONOTONIC,&tp1);long rtt=(tp1.tv_sec-tp0.tv_sec)*1000L+(tp1.tv_nsec-tp0.tv_nsec)/1000000L;
        if(rtt>0)push_log("I","ping_ms=%ld",rtt);last_ping=now;
    }
    return NULL;
}
...
static void *main_thread(void *arg){
    ...
    while(g_running){
        ...
        if(bind(ufd,(struct sockaddr*)&ula,sizeof(ula))<0){close(ufd);close(rfd);close(tfd_tcp);close(tfd_udp);push_log("E","udp bind failed");sleep(2);continue;}
        ...
        reader_arg_t *ra_rd=malloc(sizeof(reader_arg_t));if(ra_rd){ra_rd->tfd=tfd_tcp;ra_rd->epoch=epoch;}
        reader_arg_t *ra_ka=malloc(sizeof(reader_arg_t));if(ra_ka){ra_ka->tfd=tfd_tcp;ra_ka->epoch=epoch;}
        udp_srv_arg_t *ra_ur=malloc(sizeof(udp_srv_arg_t));if(ra_ur){ra_ur->tfd=tfd_udp;ra_ur->epoch=epoch;}
        ...
        while(g_running){
            ...
            if(pr==0){pthread_mutex_lock(&g_state_mu);int same=(g_tun_tcp==tfd_tcp&&g_relay_fd==rfd);pthread_mutex_unlock(&g_state_mu);if(!same)break;continue;}
            ...
            if(req[1]==0x03){
                /* UDP ASSOCIATE — despachar a thread dedicado */
                /* Ya leímos el request, handle_udp_associate espera leer el request */
                /* Así que lo manejamos inline aquí */
                uint8_t reply[10]={0x05,0x00,0x00,0x01,127,0,0,1,(UDP_RELAY_PORT>>8)&0xFF,UDP_RELAY_PORT&0xFF};
                send(cfd,reply,sizeof(reply),MSG_NOSIGNAL);
                ...
                /* CONNECT (0x01) — conexión TCP normal */
                /* Re-ensamblar: los workers necesitan procesar desde el principio */
                /* Como ya consumimos el greeting + auth + request, reenviamos el request al handler */
                /* El handler espera datos del socket local hacia el destino */
                /* En nuestro caso el worker solo lee datos para mandárselos al servidor */
                /* El SOCKS5 CONNECT response lo mandamos aquí */
    ...
    for(int i=0;i<g_workers_n;i++)pthread_join(g_workers[i],NULL);g_workers_n=0;g_started=0;
    return NULL;
}
...
Java_com_blacktunnel_BtProxy_nativeStart(JNIEnv *env,jclass clazz,jint port,jobject svc,jstring iid){
    ...
    ht_init();g_running=1;g_started=0;g_reconnect_delay=RECONNECT_DELAY_MIN;atomic_store(&g_next_sid,1);atomic_store(&g_last_traffic,(long)time(NULL));
    pthread_mutex_unlock(&g_state_mu);pthread_mutex_lock(&g_start_mu);g_start_st=0;pthread_mutex_unlock(&g_start_mu);
    if(pthread_create(&g_main_thr,NULL,main_thread,(void*)(intptr_t)port)!=0){pthread_mutex_lock(&g_state_mu);g_running=0;(*env)->DeleteGlobalRef(env,g_vpn_svc);g_vpn_svc=NULL;g_jvm=NULL;pthread_mutex_unlock(&g_state_mu);return -1;}
    pthread_detach(g_main_thr);
    struct timespec ts;clock_gettime(CLOCK_REALTIME,&ts);ts.tv_sec+=12;pthread_mutex_lock(&g_start_mu);while(g_start_st==0)if(pthread_cond_timedwait(&g_start_cv,&g_start_mu,&ts)!=0)break;int st=g_start_st;pthread_mutex_unlock(&g_start_mu);
...
Java_com_blacktunnel_BtProxy_nativeStop(JNIEnv *env,jclass clazz){
    (void)clazz;pthread_mutex_lock(&g_state_mu);if(!g_running){pthread_mutex_unlock(&g_state_mu);return;}
    ...
    pthread_mutex_lock(&g_q_mu);pthread_cond_broadcast(&g_q_cv);pthread_mutex_unlock(&g_q_mu);
    pthread_mutex_lock(&g_start_mu);if(g_start_st==0){g_start_st=-1;pthread_cond_broadcast(&g_start_cv);}pthread_mutex_unlock(&g_start_mu);
    for(int i=0;i<20&&g_started;i++)usleep(10000);ht_clear();if(svc)(*env)->DeleteGlobalRef(env,svc);g_started=0;
}
...
JNIEXPORT jstring JNICALL Java_com_blacktunnel_BtProxy_nativeDrainLogs(JNIEnv *env,jclass clazz){(void)clazz;pthread_mutex_lock(&g_log_mu);if(g_log_len==0){pthread_mutex_unlock(&g_log_mu);return (*env)->NewStringUTF(env,"");}char out[32768];memcpy(out,g_log_buf,g_log_len);out[g_log_len]='\0';g_log_len=0;g_log_buf[0]='\0';pthread_mutex_unlock(&g_log_mu);return (*env)->NewStringUTF(env,out);}
