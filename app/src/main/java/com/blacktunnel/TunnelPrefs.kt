...
object TunnelPrefs {
    ...
    private const val DEFAULT_VLESS_UUID = "a3482e88-686a-4a58-8126-99c9df64b7bf"
    ...
    fun getVlessUuid(ctx: Context): String =
        ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .getString(KEY_VLESS_UUID, DEFAULT_VLESS_UUID)
            ?.trim()
            .orEmpty()
            .ifBlank { DEFAULT_VLESS_UUID }

    fun setVlessUuid(ctx: Context, uuid: String) {
        val value = uuid.trim().ifBlank { DEFAULT_VLESS_UUID }
        ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString(KEY_VLESS_UUID, value).apply()
    }
