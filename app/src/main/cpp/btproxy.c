...
static void call_java(const char *method, const char *sig) {
    JavaVM *jvm; jobject svc;
    pthread_mutex_lock(&g_mu); jvm=g_jvm; svc=g_svc; pthread_mutex_unlock(&g_mu);
    if (!jvm||!svc) return;
    ...
    while (g_running) {
        ...
        for (int i=0;i<2;i++) {
            ...
            if (idx==1) {
                ...
                if (n>0) {
                    ssize_t off=0;
                    while(off<n){ssize_t w=send(cfd,buf+off,n-off,MSG_NOSIGNAL);
                        if(w>0){off+=w;continue;}if(errno==EINTR)continue;goto done;}
...
static void *keepalive(void *arg) {
    ...
    while (g_running && atomic_load(&g_tunnel_epoch)==epoch) {
        ...
        for (int us=5000,el=0;el<3000000&&g_running;el+=us) {
            if (atomic_load(&g_last_pong)>pong) break;
...
static void *main_thread(void *arg) {
    ...
    while (g_running) {
        ...
        if (!is_first) call_java("onTunnelReconnected","()V");
        is_first=0;
