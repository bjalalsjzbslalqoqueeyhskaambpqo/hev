...
static void *tunnel_reader(void *arg) {
    ...
    while (g_running) {
        if (tun_recv(tfd, hdr, FRAME_HDR) < 0) { request_tunnel_reset("tunnel header read failed"); break; }
        ...
        if (len > MAX_PAYLOAD) { request_tunnel_reset("tunnel payload too large"); break; }
        if (len > 0 && tun_recv(tfd, payload, len) < 0) { request_tunnel_reset("tunnel payload read failed"); break; }
    ...
    while (g_running) {
        ...
        while (g_running) {
            ...
            if (cfd < 0) {
                ...
                request_tunnel_reset("accept failed");
                break;
            }
            sock_tune(cfd);
