...
static stream_t *ht_get(uint32_t sid) {
    int s = (int)(sid & HT_MASK);
    pthread_mutex_lock(&g_ht_mu[s]);
    stream_t *n = g_ht[s];
...
static stream_t *ht_put(uint32_t sid, int fd) {
    stream_t *n = malloc(sizeof(stream_t));
    if (!n) return NULL;
    n->sid = sid; n->fd = fd;
    int s = (int)(sid & HT_MASK);
    pthread_mutex_lock(&g_ht_mu[s]);
    n->next = g_ht[s]; g_ht[s] = n;
    pthread_mutex_unlock(&g_ht_mu[s]);
    return n;
}
...
static task_t *q_pop(void) {
    pthread_mutex_lock(&g_q_mu);
    while (g_q_head == NULL && g_running) pthread_cond_wait(&g_q_cv, &g_q_mu);
    if (!g_q_head) { pthread_mutex_unlock(&g_q_mu); return NULL; }
    task_t *t = g_q_head; g_q_head = t->next;
...
static int tun_send(uint8_t type, uint32_t sid, const uint8_t *data, uint16_t dlen) {
    ...
    while (sent < total) {
        ...
        if (n > 0) {
            ...
            struct pollfd p = { g_tun_fd, POLLOUT, 0 };
            if (poll(&p, 1, 30) <= 0) { pthread_mutex_unlock(&g_tun_wmu); return -1; }
        } else {
            pthread_mutex_unlock(&g_tun_wmu); return -1;
        }
    }
...
static int open_tunnel(void) {
    ...
    if (recv_headers(fd, h1, sizeof(h1), HANDSHAKE_TIMEOUT_SEC) < 0) {
        push_log("E", "proxy h1 read failed"); close(fd); return -1;
    }
    ...
    int h2len = recv_headers(fd, h2, sizeof(h2), HANDSHAKE_TIMEOUT_SEC);
    int status = -1; sscanf(h2, "HTTP/%*d.%*d %d", &status);
    if (h2len < 0 || status != 101) {
        const char *body = ""; char tmp[512] = {0};
        const char *eoh = strstr(h2, "\r\n\r\n");
        if (eoh) {
            int bl = h2len - (int)(eoh + 4 - h2);
            if (bl > 0 && bl < (int)sizeof(tmp)) { memcpy(tmp, eoh + 4, bl); tmp[bl] = '\0'; body = tmp; }
        }
        ...
        else                                                                       push_log("E", "auth error %d", status);
        close(fd); return -1;
    }
    clock_gettime(CLOCK_MONOTONIC, &t1);
    long ping_ms = (t1.tv_sec - t0.tv_sec) * 1000L + (t1.tv_nsec - t0.tv_nsec) / 1000000L;
    char uname[128] = {0}, udays[32] = {0}; char *p;
    if ((p = strstr(h2, "X-User-Name:")) != NULL) { sscanf(p + 12, " %127[^\r\n]", uname); push_log("I", "user=%s", uname); }
    if ((p = strstr(h2, "X-User-Days:")) != NULL) { sscanf(p + 12, " %31[^\r\n]",  udays); push_log("I", "days=%s", udays); }
    push_log("I", "ping=%ldms gaming=%d", ping_ms, gaming);
    return fd;
}

static void handle_conn(int cfd, int tfd, uint32_t sid) {
    (void)tfd;
    ...
    if (tun_send(T_OPEN, sid, buf, (uint16_t)n) < 0) {
        tunnel_reset("T_OPEN failed"); ht_del(sid); return;
    }
    ...
    while (g_running) {
        ...
        if (n == 0) break;
        atomic_store(&g_last_traffic, (long)time(NULL));
        if (tun_send(T_DATA, sid, buf, (uint16_t)n) < 0) {
            tunnel_reset("T_DATA failed"); break;
        }
    }

    tun_send(T_CLOSE, sid, NULL, 0);
    ht_del(sid);
}
...
static void *worker(void *arg) {
    ...
    while (g_running) {
        task_t *t = q_pop();
        if (!t) break;
        handle_conn(t->cfd, t->tfd, t->sid);
        free(t);
    }
    return NULL;
}

typedef struct { int tfd; int epoch; } thr_arg_t;

static void *tunnel_reader(void *arg) {
    thr_arg_t *ta = (thr_arg_t*)arg; int tfd = ta->tfd, epoch = ta->epoch; free(ta);
    ...
    while (g_running && atomic_load(&g_epoch) == epoch) {
        ...
        if (rc == -2) {
            if ((long)time(NULL) - atomic_load(&g_last_pong) > PONG_TIMEOUT_SEC) {
                tunnel_reset("pong timeout"); break;
            }
            continue;
        }
        ...
        uint8_t  ft  = hdr[0];
        uint32_t sid = ((uint32_t)hdr[1] << 24) | ((uint32_t)hdr[2] << 16) | ((uint32_t)hdr[3] << 8) | hdr[4];
        ...
        case T_DATA: {
            ...
            ssize_t off = 0;
            while (off < len) {
                ssize_t w = send(s->fd, payload + off, len - off, MSG_NOSIGNAL);
...
static void *keepalive(void *arg) {
    thr_arg_t *ta = (thr_arg_t*)arg; int epoch = ta->epoch; free(ta);
    long last_ping = 0;
    ...
    while (g_running && atomic_load(&g_epoch) == epoch) {
        ...
        int gaming = atomic_load(&g_gaming);
        int ka_sec = gaming ? 5 : KEEPALIVE_SEC;
...
static void *main_thread(void *arg) {
    ...
    while (g_running) {
        ...
        thr_arg_t *ra_rd = malloc(sizeof(thr_arg_t));
        thr_arg_t *ra_ka = malloc(sizeof(thr_arg_t));
        if (ra_rd) { ra_rd->tfd = tfd; ra_rd->epoch = epoch; }
        if (ra_ka) { ra_ka->tfd = tfd; ra_ka->epoch = epoch; }
        pthread_t trd, tka;
        if (ra_rd) { pthread_create(&trd, NULL, tunnel_reader, ra_rd); pthread_detach(trd); } else free(ra_rd);
        ...
        while (g_running) {
            ...
            uint8_t no_auth[] = { 0x05, 0x00 };
            send(cfd, no_auth, 2, MSG_NOSIGNAL);
            ...
            if (bn > req_offset) {
                rn = bn - req_offset;
                if (rn > (ssize_t)sizeof(req)) rn = (ssize_t)sizeof(req);
                memcpy(req, buf + req_offset, rn);
            }
            while (rn < 4) {
                ssize_t got = recv(cfd, req + rn, sizeof(req) - (size_t)rn, 0);
                if (got <= 0) break;
                rn += got;
            }
            if (rn < 4 || req[0] != 0x05 || req[1] != 0x01) { close(cfd); continue; }
            int need = 0;
            if      (req[3] == 0x01) need = 4 + 2;
            else if (req[3] == 0x03) need = (rn > 4) ? (int)req[4] + 1 + 2 : 256;
            else if (req[3] == 0x04) need = 16 + 2;
            if (need > 0) {
                int full = 4 + need;
                while (rn < full) {
                    ssize_t got = recv(cfd, req + rn, sizeof(req) - (size_t)rn, 0);
...
Java_com_blacktunnel_BtProxy_nativeApplyMode(JNIEnv *e, jclass c, jboolean en) {
    (void)e; (void)c; (void)en;
    q_clear(); ht_clear();
    push_log("I", "mode applied");
}
