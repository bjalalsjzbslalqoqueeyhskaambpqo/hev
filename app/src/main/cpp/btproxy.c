...
#define MAX_WORKERS          64      /* tamaño fijo del pool */
#define ACCEPT_POLL_MS       10     /* latencia máxima para aceptar nuevas conexiones */
#define KEEPALIVE_SEC        10      /* intervalo entre pings */
#define PONG_TIMEOUT_SEC     180     /* tiempo máximo sin pong antes de reconectar */
#define PING_IDLE_SEC        120     /* ping más espaciado cuando no hay tráfico */
#define IDLE_TRAFFIC_SEC     30      /* segundos sin datos para considerar idle */
...
/* ── Cola de tareas para el pool de workers ──────────────────────────────── */
...
static task_t         *g_q_head = NULL;
static task_t         *g_q_tail = NULL;
...
static int http_status(const char *h) {
    int code = -1; sscanf(h, "HTTP/%*d.%*d %d", &code); return code;
}
...
static int open_tunnel(void) {
    ...
    if (fd < 0) {
        ...
        if (getaddrinfo(PROXY_HOST, ps, &hints, &res) == 0) {
            for (struct addrinfo *r = res; r && fd < 0; r = r->ai_next) {
                ...
                if (s < 0) continue;
                jni_protect(s);
                int v = 1; setsockopt(s, IPPROTO_TCP, TCP_NODELAY, &v, sizeof(v));
    ...
    if (hlen < 0 || http_status(h2) != 101) {
        ...
        if (strstr(h2, "not_registered") || strstr(body, "not_registered") ||
            ...
            push_log("E", "auth error %d", http_status(h2));
        close(fd); return -1;
    }
...
static void handle_conn(int cfd, int tfd, uint32_t sid) {
    ...
    atomic_store(&g_last_traffic, (long)time(NULL));
    if (tun_send(tfd, T_OPEN, sid, buf, (uint16_t)n) < 0) {
...
typedef struct { int tfd; int epoch; } reader_args_t;
...
static void *keepalive(void *arg) {
    ...
    while (g_running && atomic_load(&g_epoch) == epoch) {
        ...
        int interval = idle ? PING_IDLE_SEC : KEEPALIVE_SEC;
        if (now - last_ping < interval) continue;

        struct timespec tp0, tp1;
        clock_gettime(CLOCK_MONOTONIC, &tp0);
        long prev_pong = atomic_load(&g_last_pong);

        if (tun_send(tfd, T_PING, 0, NULL, 0) < 0) { tunnel_reset("ping failed"); break; }
...
static void *main_thread(void *arg) {
    ...
    while (g_running) {
        ...
        push_log("I", "relay ready port=%d epoch=%d", port, epoch);
        ...
        reader_args_t *ra_rd = malloc(sizeof(reader_args_t));
        reader_args_t *ra_ka = malloc(sizeof(reader_args_t));
